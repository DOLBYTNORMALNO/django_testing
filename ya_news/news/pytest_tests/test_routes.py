...
@pytest.mark.django_db
def test_anonymous_user_cannot_edit_comment(client, user_fixture, news_fixture, comment_fixture):
...
@pytest.mark.django_db
def test_anonymous_user_cannot_delete_comment(client, user_fixture, news_fixture, comment_fixture):
    comment = comment_fixture
    delete_response = client.get(reverse(NEWS_DELETE, kwargs={"pk": comment.pk}))
    assert delete_response.status_code == 302


@pytest.mark.django_db
def test_user_cannot_edit_or_delete_other_comments(client, user_fixture, another_user_fixture, comment_fixture):
